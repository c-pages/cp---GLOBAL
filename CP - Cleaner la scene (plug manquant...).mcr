macroScript CleanerDeScene
category:"#CPages"
	toolTip:"Cleaner De Scene (plug manquant et autre trucs )"
(



	fn fn_Clean_MissingDll =(
	...
	) -- fin 	fn fn_Clean_MissingDll 
	
	fn fn_Clean_CA = (
		thescene = (refs.dependents rootnode)[1]
		custAttributes.delete	 thescene 1
	) -- fin fn_Clean_CA
	...
	fn	fn_clean_RendererMR = (
			-- moteur rendu -->  mentalray  
			arraytemp = filterstring (renderers.current as string) ":"
			NomSource = arraytemp[1]
			NomDest = "mental_ray_renderer"
			if (NomDest != NomSource) do (
				renderers.current = mental_ray_renderer ()
			)
			renderers.current.UseFinalGatherFile = false
			renderers.current.FinalGatherBounces = 2
			renderers.current.GlobalIllumEnable = false
		)
	
	if ( queryBox "Cleanner la scène ? (gIvy, missing DLL, CA )" ) do (
		fn_clean_Ivy ()
		fn_Clean_CA ()
		fn_Clean_MissingDll ()
		fn_clean_RendererMR()
